...
const AVATAR_STILL_CUTOUTS = {
  "306": "./digital_human_images/306_cutout_v8.png",
};
...
let avatars = [
  {
    ...
    ttsSpeakerId: "6224",
    name: "Evan Hart",
    role: "Calm, structured, and emotionally steady.",
    speakerLabel: "6224 · voice",
    color: "#32d0a4",
    reply: "",
    imageUrl: "./digital_human_images/306_ce51edb3.png"
  },
  { id: "2001", avatarId: "2001", ttsSpeakerId: "1801", name: "Marco Leone", role: "High-energy, expressive, and opinion-driven.", speakerLabel: "1801 · voice", color: "#ffb84d", reply: "", imageUrl: "./digital_human_images/2001_ce093d24.png" },
  { id: "2002", avatarId: "2002", ttsSpeakerId: "3553", name: "Aria Vale", role: "Soft-spoken, empathetic, and emotionally perceptive.", speakerLabel: "3553 · voice", color: "#a78bfa", reply: "", imageUrl: "./digital_human_images/2002_8fdefcf3.png" },
  { id: "2003", avatarId: "2003", ttsSpeakerId: "6346", name: "Darius Kane", role: "Direct, assertive, and decision-driven.", speakerLabel: "6346 · voice", color: "#22c55e", reply: "", imageUrl: "./digital_human_images/2003_9d2fcfc3.png" }
];
...
function avatarStillImageUrl(item) {
  const cutout = AVATAR_STILL_CUTOUTS[String(item?.avatarId || item?.id || "")];
  return cutout || item?.imageUrl || "";
}
  ...
  document.querySelector("#app").innerHTML = `
    <main class="page login-page">
      <div class="cosmos-scene" aria-hidden="true">
        ...
        <div class="orbit orbit-tight"></div>
        <span class="planet planet-aurora" data-rx="390" data-ry="92" data-speed="0.00018" data-phase="2.65"></span>
        <span class="planet planet-coral" data-rx="300" data-ry="68" data-speed="-0.00024" data-phase="2.2"></span>
        <span class="planet planet-ice" data-rx="210" data-ry="46" data-speed="0.00032" data-phase="5.05"></span>
      </div>
...
function initLoginCosmos() {
  const page = document.querySelector(".login-page");
  if (!page) return;
  const scene = page.querySelector(".cosmos-scene");
  const planets = Array.from(page.querySelectorAll(".planet"));
  ...
  const updateScene = (event) => {
    ...
    page.style.setProperty("--scene-x", x.toFixed(3));
    page.style.setProperty("--scene-y", y.toFixed(3));
  };

  page.addEventListener("pointermove", updateScene);
  page.addEventListener("pointerleave", () => {
    page.style.setProperty("--scene-x", "0");
    page.style.setProperty("--scene-y", "0");
  });

  if (!scene || reduceMotion) return;

  const animatePlanets = (time) => {
    if (!scene.isConnected) return;
    const widthScale = Math.min(1, page.clientWidth / 1280);

    planets.forEach((planet) => {
      const rx = Number(planet.dataset.rx || 0) * widthScale;
      const ry = Number(planet.dataset.ry || 0) * widthScale;
      const speed = Number(planet.dataset.speed || 0);
      const phase = Number(planet.dataset.phase || 0);
      const angle = phase + time * speed;
      const yRatio = Math.sin(angle);
      const x = Math.cos(angle) * rx;
      const y = yRatio * ry;
      const depthScale = 0.78 + (yRatio + 1) * 0.16;

      planet.style.setProperty("--planet-x", `${x.toFixed(2)}px`);
      planet.style.setProperty("--planet-y", `${y.toFixed(2)}px`);
      planet.style.setProperty("--planet-scale", depthScale.toFixed(3));
      planet.style.zIndex = yRatio > 0 ? "2" : "0";
    });
...
function renderRoom() {
  ...
  const avatarStillUrl = versionedImageUrl(avatarStillImageUrl(avatar), avatar.imageVersion || "1");
  ...
  document.querySelector("#app").innerHTML = `
    <main class="page">
      ${topbar("EmpaAva", session, { back: true })}
      <section class="room">
        ...
        <article class="video-pane">
          <div class="video-frame">
            ...
            <div class="avatar-stage avatar-bg-${escapeHtml(background.id)} ${background.imageUrl ? "has-custom-bg" : ""}" style="--avatar-color: ${avatar.color}; ${avatarBackgroundStyle(background)}">
              ${
                avatarVideoUrl
                  ? `<video id="avatarVideo" class="avatar-video" src="${escapeHtml(avatarVideoUrl)}" preload="auto" playsinline controls ${activeAvatarView === "video" ? "" : "hidden"}></video><canvas id="avatarVideoCanvas" class="avatar-video-canvas" ${activeAvatarView === "video" ? "" : "hidden"}></canvas>`
                  : ""
              }
              ...
              <div class="avatar-person ${avatarStillUrl ? "has-still" : ""}" aria-hidden="true" ${avatarVideoUrl ? "hidden" : ""}>
                ${
                  avatarStillUrl
                    ? `<img class="avatar-still" src="${escapeHtml(avatarStillUrl)}" alt="" />`
...
async function sendTalk() {
  ...
  if (PUBLIC_DEMO_MODE) {
    setGenerationProgress(28, "Analyzing your video and voice");
    await new Promise((resolve) => setTimeout(resolve, 650));
    setGenerationProgress(55, "Planning emotional response");
    await new Promise((resolve) => setTimeout(resolve, 650));
    setGenerationProgress(82, "Driving avatar motion");
    await new Promise((resolve) => setTimeout(resolve, 650));
    await applyAvatarResult({
